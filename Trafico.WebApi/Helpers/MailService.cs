    ...
    public class MailService
    {
        ...
        public bool Enviar_Correo(string Receptor, string asunto, string mensaje)
        {
            try
            {
                ...
                string inspectorUser = configuration["Mail_inspectorUser"];

                // --- Obteniendo lista de correos
                string[] liCorreosIns = inspectorUser.Split(";");
                ...
                // --- Agregando lista de inspectores de correo
                foreach (string correo in liCorreosIns)
                {
                    mail.To.Add(new MailAddress(correo));
                }
